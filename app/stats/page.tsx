...
export default async function StatsPage() {
  ...
  return (
    <>
      <PageHero eyebrow="Analytics" title="Statistics" subtitle="2026 season · live numbers after each game" />
      <div className="page-shell-wide space-y-8">
        {!hasAny ? (
          <div className="card-feature overflow-hidden">
            <div className="section-dark px-6 py-8">
              <p className="hero-eyebrow">Preseason</p>
          ...
          <>
            <section>
              <h3 className="mb-3 label-cap">Team</h3>
              {highlights.length === 0 ? (
                <p className="empty-state">Team totals appear after the first games.</p>
              ) : (
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
                  {highlights.map((h) => (
                    <div key={h.name} className="card-feature p-4">
...
function LeaderBlock({
  ...
  return (
    <section>
      <h3 className="mb-3 label-cap">{title}</h3>
      {rows.length === 0 ? (
        <p className="empty-state">No data yet.</p>
      ) : (
        <div className="card-feature overflow-hidden">
          {rows.map((r, i) => (
            <div key={`${r.player}-${i}`} className="row-divide flex items-center justify-between px-4 py-3">
              <span className="font-medium">{r.player}</span>
              <span className="font-semibold text-cardinal">
                {r.stat} <span className="text-xs font-normal text-ink-faint">{r.statType}</span>
              </span>
            </div>
          ))}
        </div>
